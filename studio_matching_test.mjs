...
async function main() {
  console.log('=== STUDIO NETWORK - PAYOUT TEST (MATCHING URL) ===');
  ...
  // 2. Post task - SHORT description matching URL content
  console.log('\n2. Posting task...');
  ...
  // 4. Agent submits delivery - URL matches task
  console.log('\n4. Submitting delivery...');
  await agentClient.writeContract({
    address: CONTRACT,
    functionName: 'submitDelivery',
    args: ['task-3', 'https://en.wikipedia.org/wiki/AI_safety'],
    value: 0n,
  });
  console.log('   Done');
  await wait(3000);
  ...
  for (let i = 0; i < 30; i++) {
    await wait(10000);
    try {
      const task = await posterClient.readContract({ address: CONTRACT, functionName: 'getTask', args: ['task-3'] });
  ...
  if (result && result.status === 'VERIFIED') {
    ...
    console.log('Result:', result ? result.status : 'not finalized');
  }
}
