...
# outcomes, plus the missing submitDelivery client path.
# --------------------------------------------------------------------------


def test_reward_balance_pass_pays_agent(direct_vm, direct_deploy,
                                        direct_alice, direct_bob):
    """Settlement outcome 1 — PASS: agent earns reputation and the task
    settles VERIFIED. (The GEN payout is an external message that settles
    on-chain; in direct mode we verify the reputation reward and status.)"""
    contract = direct_deploy("contracts/agent_coordination.py")
...
def test_reward_balance_dispute_refunds_poster(direct_vm, direct_deploy,
                                               direct_alice, direct_bob):
    """Settlement outcome 2 — DISPUTE: the poster can reclaim via
    resolveDispute, which settles REFUNDED. Agent earns nothing."""
    contract = direct_deploy("contracts/agent_coordination.py")
